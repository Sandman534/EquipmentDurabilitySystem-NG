...
namespace EDUI {
	...
	void __stdcall RenderRates() {
		...
		if (CollapsingHeader(Translate("Break.Options"), ImGuiTreeNodeFlags_DefaultOpen)) {
			if (Checkbox(Translate("Break.Disable"), &Settings->ED_BreakDisabled))
				changeRateOption = true;
			if (Checkbox(Translate("Break.Remove"), &Settings->ED_RemoveEquipment))
				changeRateOption = true;
	...
	void __stdcall RenderMaterial() {
		...
		if (CollapsingHeader(Translate("Mitigation.Material"), ImGuiTreeNodeFlags_DefaultOpen)) {
			if (Checkbox(Translate("Mitigation.MaterialEnable"), &Settings->ED_Material_Multiplier))
				changeMaterialOption = true;

			if (BeginTable("##MaterialRates", 2)) {
				TableSetupColumn(Translate("Mitigation.MaterialName"), ImGuiTableColumnFlags_WidthFixed, 150.0f);
				TableSetupColumn(Translate("Mitigation.MaterialRate"));
				TableHeadersRow();

				if (MaterialEntry(Translate("Mitigation.Fur"), Settings->ED_Fur))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Leather"), Settings->ED_Leather))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Iron"), Settings->ED_Iron))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Steel"), Settings->ED_Steel))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Silver"), Settings->ED_Silver))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Elven"), Settings->ED_Elven))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Bonemold"), Settings->ED_Bonemold))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Falmer"), Settings->ED_Falmer))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Chitin"), Settings->ED_Chitin))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Dwarven"), Settings->ED_Dwarven))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Glass"), Settings->ED_Glass))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Orcish"), Settings->ED_Orcish))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Stalhrim"), Settings->ED_Stalhrim))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Ebony"), Settings->ED_Ebony))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Dragon"), Settings->ED_Dragon))
					changeMaterialOption = true;
				if (MaterialEntry(Translate("Mitigation.Daedric"), Settings->ED_Daedric))
					changeMaterialOption = true;
				EndTable();
			}
		}
	...
	bool CreateInputText(const char* label, std::string& str, ImGuiInputTextFlags flags) {
		bool changed = false;
